...
router.post('/', publicFormLimiter, optionalAuth, async (req, res) => {
  try {
    const { email, firstName, lastName, projectId, slug, amount, frequency, paymentMethod } = req.body;

    const numAmount = Number(amount);
    if (isNaN(numAmount) || numAmount <= 0) {
      return res.status(400).json({ success: false, message: 'Le montant du don doit être supérieur à 0.' });
    }
    ...
    if (!targetProject && slug) {
      const found = await db.select().from(projects).where(eq(projects.slug, slug));
      if (found.length > 0) targetProject = found[0];
    }
    if (!targetProject) {
    ...
    if (!targetProject) {
      return res.status(400).json({ success: false, message: 'Projet introuvable.' });
    }
    ...
    if (!userId && donorEmail) {
      ...
      if (existing.length > 0) {
        ...
        const tempPassword = 'GuestPass_' + Math.random().toString(36).slice(-8);
    ...
    const formattedDate = new Date(newDonation.createdAt).toISOString().slice(0, 10);
    ...
    return res.status(500).json({ success: false, message: 'Erreur serveur lors de la création du don.' });
  }
});
...
router.get('/export.csv', authenticateUser, requireAdmin, async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Erreur génération CSV.' });
  }
});
